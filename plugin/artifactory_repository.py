...
class JFrogArtifactoryRepository(ArtifactRepository):
    ...
    def __init__(self, artifact_uri, tracking_uri=None):
        ...
        self.artifacts_delete_skip = os.getenv(
            "ARTIFACTORY_ARTIFACTS_DELETE_SKIP", "false"
        )
        if self.artifacts_delete_skip.lower() == "false":
            print(
                "JFrog: Deletions of experiments or runs will result in artifacts deletions on Artifactory"
            )
        else:
            print(
                "JFrog: Deletions of experiments or runs will not result in artifacts deletions on Artifactory"
            )
        if self.debug_mode:
            print("JFrog: uri = {} repo={}".format(self.rt_url, self.repository))
        if self.token is None:
            raise Exception(
                "No Artifactory Token provided through environment ARTIFACTORY_AUTH_TOKEN`"
            )
    ...
    def extract_uri(uri, rt_no_ssl):
        ...
        if (
            len(path_segments) < 2
            or path_segments[0] != "artifactory"
    ...
    def log_artifact(self, local_file, artifact_path=None):
        if self.debug_mode:
            print(
                "JFrog: log_artifact localfile={} , artifact_path={}".format(
        ...
        with open(local_file, "rb") as f:
            requests.put(
                self.rt_url + "/" + self.repository + "/" + dest_path,
                data=f,
    ...
    def log_artifacts(self, local_dir, artifact_path=None):
        if self.debug_mode:
            print(
                f"JFrog: log_artifacts parameters local_dir={local_dir}, artifact_path={artifact_path}"
            )
        verify_artifact_path(artifact_path)
        ...
        for root, _, filenames in os.walk(local_dir):
            ...
            for f in filenames:
                with open(os.path.join(root, f), "rb") as file:
                    if self.debug_mode:
                        print(
                            f"JFrog: log_artifacts log_artifacts.file {file} put into path {upload_path}"
                        )
                    if upload_path:
                        url = (
                            self.rt_url
                    ...
                    if self.debug_mode:
                        print(
                            f"JFrog: log_artifacts put status {r.status_code} reason {r.reason}"
                        )
    ...
    def list_artifacts(self, path=None):
        ...
        r = requests.get(
            self.rt_url + "/api/storage/" + self.repository + path,
            headers=self.get_headers(),
        )
        ...
        r = requests.get(
            ...
            + self.repository
            + path
            + "?list&deep=0&depth=1&listFolders=1",
            headers=self.get_headers(),
        )
        if r.status_code != 200:
            raise Exception(f"Error: {r.status_code} {r.reason}")
    ...
    def _download_file(self, remote_file_path, local_path):
        print(
            f"JFrog: _download_file in remote_file_path={remote_file_path}, local_path={local_path} "
        )

        with requests.get(
            self.rt_url + "/" + self.repository + "/" + remote_file_path,
            headers=self.get_headers(),
            stream=True,
        ) as r:
            r.raise_for_status()
    ...
    def delete_artifacts(self, artifact_path=None):
        print(
            f"JFrog: delete skip in artifact_path={artifact_path}, is set to {self.artifacts_delete_skip}"
        )
        if self.debug_mode:
            print("JFrog: uri = {} repo={}".format(self.rt_url, self.repository))
        ...
        with requests.delete(dest_path, headers=self.get_headers(), stream=True) as r:
            r.raise_for_status()
